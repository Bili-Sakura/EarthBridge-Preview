...
#
# Scheduler for Diffusion Bridge Implicit Models (DBIM) compatible with
# the Hugging Face diffusers library.
# Based on: https://arxiv.org/abs/2405.15885
...
class DBIMSchedulerOutput(BaseOutput):
    """
    Output class for DBIM scheduler step functions.
...
class DBIMScheduler(SchedulerMixin, ConfigMixin):
    ...
    Args:
        ...
        sigma_max (`float`, defaults to `1.0`):
            Maximum time/noise level. For VP schedules this is usually 1.
        ...
        pred_mode (`str`, defaults to `"vp"`):
            Bridge noise mode (`"vp"` or `"ve"`; `*_simple` variants are also
            accepted and treated as their base mode).
        num_train_timesteps (`int`, defaults to `40`):
            Training-time nominal number of steps.
        sampler (`str`, defaults to `"dbim"`):
        ...
        order (`int`, defaults to `2`):
            Solver order for `dbim_high_order` (2 or 3).
        lower_order_final (`bool`, defaults to `True`):
            Use lower-order final update for stability in high-order sampling.
    """
    ...
    def get_f_g2(self, t: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """Return drift coefficient `f(t)` and diffusion coefficient squared `g(t)^2`."""
        t = torch.as_tensor(t, dtype=torch.float32)
        mode = self._mode()
        if mode == "vp":
            f = -0.5 * self.beta_min - 0.5 * self.beta_d * t
            g2 = self.beta_min + self.beta_d * t
            return f, g2
        # ve
        return torch.zeros_like(t), 2.0 * t

    def get_alpha_rho(
        self, t: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """Return `(alpha_t, alpha_bar_t, rho_t, rho_bar_t)` for the bridge process."""
        t = torch.as_tensor(t, dtype=torch.float32)
        mode = self._mode()
    ...
    def step(
        ...
        """Single first-order DBIM step from index `timestep` to `timestep + 1`."""
