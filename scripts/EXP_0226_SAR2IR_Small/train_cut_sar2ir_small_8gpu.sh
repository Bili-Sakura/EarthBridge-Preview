#!/usr/bin/env bash
# EXP-0226 — CUT — SAR→IR — Medium tier (~14.1 M params)
#
# Same small architecture as CUT SAR2EO (0217) that succeeded. Trains at 512×512
# (crop from 1024). Uses SwanLab for logging.
#
# Usage:
#   bash scripts/EXP_0226_SAR2IR_Small/train_cut_sar2ir_small_8gpu.sh
#   NGPU=8 bash scripts/EXP_0226_SAR2IR_Small/train_cut_sar2ir_small_8gpu.sh
...
LOG_FILE="${LOG_DIR}/train_cut_sar2ir_small_8gpu.log"
mkdir -p "${LOG_DIR}"

# CUT medium from configs/model_scaling_variants.yaml (~14.1 M)
OUTPUT_DIR="./ckpt/EXP_0226_SAR2IR_Small/cut/sar2ir_small_512"
NGF=64
NDF=64
N_LAYERS_D=3
...
SWANLAB_EXPERIMENT_NAME="exp-0226-cut-sar2ir-small-512"
SWANLAB_DESCRIPTION="EXP-0226 CUT SAR→IR Small (512, ~14.1M, medium tier)"
SWANLAB_TAGS="cut,exp-0226,sar2ir,small"
...
if [ "${NGPU}" -gt 1 ]; then
  echo "EXP-0226 CUT SAR2IR small — ${NGPU} GPUs. Log: ${LOG_FILE}"
  nohup accelerate launch --num_processes "${NGPU}" -m examples.cut.train_sar2ir \
    "${ARGS[@]}" > "${LOG_FILE}" 2>&1 &
else
  echo "EXP-0226 CUT SAR2IR small — 1 GPU. Log: ${LOG_FILE}"
  nohup python -m examples.cut.train_sar2ir \
    "${ARGS[@]}" > "${LOG_FILE}" 2>&1 &
fi
