...
LOG_DIR="./logs"
LOG_FILE="${LOG_DIR}/train_sar2eo_cut_large.log"
mkdir -p "${LOG_DIR}"

# CUT large from configs/model_scaling_variants.yaml
OUTPUT_DIR="./ckpt/4th-MAVIC-T-ckpt-0217/sar2eo_large"
NGF=128
NDF=128
N_LAYERS_D=3
...
# Training: 10 epochs, validate and save+push checkpoint every epoch (smaller batch for 24GB)
TRAIN_BATCH_SIZE=4
N_EPOCHS=10
N_EPOCHS_DECAY=0
MAX_TRAIN_STEPS=
...
echo "CUT SAR2EO large — log: ${LOG_FILE} (TensorBoard: ${OUTPUT_DIR}/logs). Running in background."
