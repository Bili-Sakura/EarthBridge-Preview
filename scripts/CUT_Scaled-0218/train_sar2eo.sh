#!/usr/bin/env bash
# CUT-Scaled-0218 — SAR→EO — large size (~56.5 M params).
# 256×256 task per configs/model_scaling_variants.yaml.
#
# Usage:
#   bash scripts/CUT_Scaled-0218/train_sar2eo.sh
#   CUDA_VISIBLE_DEVICES=0 bash scripts/CUT_Scaled-0218/train_sar2eo.sh
#
...
LOG_DIR="./logs"
LOG_FILE="${LOG_DIR}/train_sar2eo_cut_scaled_large.log"
mkdir -p "${LOG_DIR}"

# CUT large from configs/model_scaling_variants.yaml (256px task)
OUTPUT_DIR="./ckpt/4th-MAVIC-T-ckpt-0218/sar2eo_large"
NGF=128
NDF=128
N_LAYERS_D=3
...
SWANLAB_EXPERIMENT_NAME="cut-scaled-sar2eo"
SWANLAB_DESCRIPTION="CUT Scaled SAR→EO (256, large)"
SWANLAB_TAGS="cut,scaled,sar2eo"
...
echo "CUT SAR2EO large (scaled) — log: ${LOG_FILE} (SwanLab: ${SWANLOG_DIR}). Running in background."
