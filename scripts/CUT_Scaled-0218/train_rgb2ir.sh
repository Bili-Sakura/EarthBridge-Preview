#!/usr/bin/env bash
# CUT-Scaled-0218 — RGB→IR — huge size (~293 M params).
# 1024×1024 task per configs/model_scaling_variants.yaml.
#
# Usage:
#   bash scripts/CUT_Scaled-0218/train_rgb2ir.sh
#   CUDA_VISIBLE_DEVICES=0,1 bash scripts/CUT_Scaled-0218/train_rgb2ir.sh
#
...
LOG_DIR="./logs"
LOG_FILE="${LOG_DIR}/train_rgb2ir_cut_scaled_huge.log"
mkdir -p "${LOG_DIR}"

# CUT huge from configs/model_scaling_variants.yaml (1024px task)
OUTPUT_DIR="./ckpt/4th-MAVIC-T-ckpt-0218/rgb2ir_huge"
NGF=256
NDF=256
N_LAYERS_D=4
...
SWANLAB_EXPERIMENT_NAME="cut-scaled-rgb2ir"
SWANLAB_DESCRIPTION="CUT Scaled RGB→IR (1024, huge)"
SWANLAB_TAGS="cut,scaled,rgb2ir"
...
echo "CUT RGB2IR huge (scaled) — log: ${LOG_FILE} (SwanLab: ${SWANLOG_DIR}). Running in background."
nohup python -m examples.cut.train_rgb2ir "${ARGS[@]}" > "${LOG_FILE}" 2>&1 &
