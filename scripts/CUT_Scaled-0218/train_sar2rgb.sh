#!/usr/bin/env bash
# CUT-Scaled-0218 — SAR→RGB — huge size (~293 M params).
# 1024×1024 task per configs/model_scaling_variants.yaml.
#
# Usage:
#   bash scripts/CUT_Scaled-0218/train_sar2rgb.sh
#   CUDA_VISIBLE_DEVICES=0,1 bash scripts/CUT_Scaled-0218/train_sar2rgb.sh
#
...
OUTPUT_DIR="./ckpt/4th-MAVIC-T-ckpt-0218/sar2rgb_huge"
NGF=256
NDF=256
N_LAYERS_D=4
...
SWANLAB_EXPERIMENT_NAME="cut-scaled-sar2rgb"
SWANLAB_DESCRIPTION="CUT Scaled SAR→RGB (1024, huge)"
SWANLAB_TAGS="cut,scaled,sar2rgb"
...
echo "CUT SAR2RGB huge (scaled) — log: ${LOG_FILE} (SwanLab: ${SWANLOG_DIR}). Running in background."
