#!/usr/bin/env bash
# DBIM-Pixel-Medium-0216 — SAR→RGB — Pixel-space DBIM (no VAE)
...
#   CUDA_VISIBLE_DEVICES=3 bash scripts/DBIM_Pixel_Medium-0216/train_sar2rgb.sh
#   # (pick 0-3 to spread stages across 4 GPUs)
#   # or multi-GPU:
#   NGPU=4 bash scripts/DBIM_Pixel_Medium-0216/train_sar2rgb.sh
...
NGPU="${NGPU:-1}"
LOG_DIR="./logs"
LOG_FILE="${LOG_DIR}/train_sar2rgb_dbim.log"
...
OUTPUT_DIR="./ckpt/DBIM_Pixel_Medium-0216/sar2rgb"

# --- SwanLab ---
SWANLOG_DIR="./ckpt/swanlog"
SWANLAB_EXPERIMENT_NAME="dbim-pixel-medium-sar2rgb"
SWANLAB_DESCRIPTION="DBIM Pixel Medium SAR→RGB"
SWANLAB_TAGS="dbim,pixel,sar2rgb"

# --- Resume from checkpoint ---
# Use pre-trained checkpoint from models/ (HF download); set to "latest" to resume from output_dir
RESUME_FROM_CHECKPOINT="${RESUME_FROM_CHECKPOINT:-models/BiliSakura/4th-MAVIC-T-ckpt-0216/dbim/sar2rgb/checkpoint-100000}"
RESUME_STEP=0
if [ -n "${RESUME_FROM_CHECKPOINT}" ]; then
  STEP_PATH="${RESUME_FROM_CHECKPOINT}"
  if [ "${RESUME_FROM_CHECKPOINT}" = "latest" ]; then
    STEP_PATH=""
    for candidate_dir in "${OUTPUT_DIR}" "${OUTPUT_DIR}/dbim/sar2rgb"; do
